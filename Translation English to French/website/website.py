...
def setting_up_model(path = 'Current_Models/seq2seq_model_one_directional.pth'):
    '''
...
def setting_up_model(path = 'Current_Models/seq2seq_model_bi_directional.pth'):
    '''
    This function will  load the model and the vocabularies
    '''
    input_dim = len(english_vocab)  
    ...
    hidden_dim = 512
    num_layers = 3
    encoder = Encoder_2(input_dim, embedding_dim, hidden_dim, num_layers)
    decoder = Decoder_2(output_dim, embedding_dim, hidden_dim, num_layers)
    model = Seq2Seq_2(encoder, decoder)
    model.load_state_dict(torch.load(path,weights_only=True))
    model.eval()
    return model
model2 = setting_up_model()

def translate_to_french(model,sentence,  french_vocab_inverse = french_index_to_token ,english_vocab = english_vocab,max_src_len = 26):
    '''
...
def index():
    ...
    translated_sentence1 = translated_sentence2 = ""
    
    if request.method == 'POST':
        # Check which form was submitted
        if 'sentence1' in request.form:
            original_sentence1 = request.form['sentence1']
            translated_sentence1 = translate_to_french( model1,original_sentence1, french_index_to_token, english_vocab)
        elif 'sentence2' in request.form:
            original_sentence2 = request.form['sentence2']
            translated_sentence2 = translate_to_french(model2, original_sentence2,  french_index_to_token, english_vocab)
    
    return render_template('index.html', original_sentence1=original_sentence1, translated_sentence1=translated_sentence1, original_sentence2=original_sentence2, translated_sentence2=translated_sentence2)
