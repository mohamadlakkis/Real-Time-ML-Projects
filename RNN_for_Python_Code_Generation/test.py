...
def data_generator(input_sequences, output_sequences, batch_size):
    while True:
        for i in range(0, len(input_sequences), batch_size):
            yield (
                input_sequences[i:i + batch_size],
                output_sequences[i:i + batch_size]
            )

history = model.fit(
    data_generator(input_sequences, output_sequences, batch_size=2),
    steps_per_epoch=len(input_sequences) // 2,
    epochs=1,
